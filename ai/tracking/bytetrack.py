...
class ByteTracker:
    ...
    def active_track_ids(self) -> set[int]:
        """
        IDs of tracks the tracker is still remembering — CONFIRMED or LOST,
        i.e. not yet deleted. A LOST track is mid-occlusion-grace-period
        (see max_age), not gone. Callers that need to know whether a person
        has truly left (e.g. attendance exit counting) should use this
        instead of the confirmed-only output of update(), otherwise every
        brief occlusion is misread as a departure.
        """
        return {t.track_id for t in self._tracks if t.state in (TrackState.CONFIRMED, TrackState.LOST)}
