...
async def attendance_history(
    camera_id: str,
    days: int = Query(default=7, ge=1, le=90, description="Number of days to look back (ignored if `date` is given)"),
    date: Optional[str] = Query(default=None, description="Specific calendar day, YYYY-MM-DD — returns just that day's data"),
    ...
    Previously this endpoint only accepted `days` (a rolling lookback) —
    the dashboard's date picker was calling it with `?date=YYYY-MM-DD`,
    which FastAPI silently ignored (unrecognized query params are just
    dropped), so picking any date always returned the same rolling
    7-day window no matter what was selected. `date` is now a real,
    supported parameter.
    """
    cam = await _require_camera(camera_id, db)
    # DB columns are TIMESTAMP WITHOUT TIME ZONE — use naive UTC to avoid
    ...
    if date:
        ...
        where_clause = (
            AttendanceEvent.recorded_at >= day_start,
            AttendanceEvent.recorded_at < day_end,
        )
    else:
        since = datetime.utcnow() - timedelta(days=days)
        where_clause = (AttendanceEvent.recorded_at >= since,)
